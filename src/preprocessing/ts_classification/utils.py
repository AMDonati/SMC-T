...
from preprocessing.ts_classification.constants import UNIVARIATE_DATASET_NAMES as DATASET_NAMES
from preprocessing.ts_classification.constants import ARCHIVE_NAMES
from preprocessing.ts_classification.constants import CLASSIFIERS
from preprocessing.ts_classification.constants import ITERATIONS
from preprocessing.ts_classification.constants import MTS_DATASET_NAMES
...
def readucr(filename):
    data = np.loadtxt(filename, delimiter = ' ') # you can add a usecol parameter to not import all the columns.
    # delimiter = space in the .txt file
    Y = data[:,0]
    X = data[:,1:]
    return X, Y
