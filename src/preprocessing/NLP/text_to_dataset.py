...
def text_to_dataset(file_path, seq_len, train_split, buffer_size, batch_size):
  ...
  text_val, text_test=train_test_split(text_as_int, train_size=0.5, shuffle=False)

  num_samples_train=text_train.shape[0]
  print ('number of training examples: {}'.format(num_samples_train))
  print('number of validation examples: {}'.format(text_val.shape[0]))
  print('number of test examples: {}'.format(text_test.shape[0]))
  ...
  char_test_dataset = tf.data.Dataset.from_tensor_slices(text_test)
  ...
  test_sequences = char_test_dataset.batch(seq_len + 1, drop_remainder=True)
...
if __name__ == "__main__":
  ...
  TRAIN_SPLIT=0.8
  seq_len=50
  train_dataset, val_dataset, test_dataset, vocab_size, training_samples = text_to_dataset(file_path=file_path, seq_len=seq_len,
