...
from train.loss_functions import binary_ce_with_particules
...
# x_train_binary, x_val_binary = train_test_split(x_data_binary,
...
print('multi classes value counts', classes_data_12.value_counts())
...
def train_step(inputs, targets=None, SMC_loss=True, classic_loss=True):
  ...
  with tf.GradientTape() as tape:
    predictions, trajectories, weights = transformer(inputs=tf.squeeze(tar_inp, axis=-1),
                                                     training=True, mask=mask_transformer)

    # predictions: shape (B,P,S,C)
    # trajectories: shape (B,P,S,D) compute Z0,Z1,Z2,...,ZT
    # weights: shape (B,P,1)
    #transformer.summary()
    ...
    if transformer.task_type=='classification':
      ...
      if tf.shape(predictions)[-1]==2:
        ...
        print('multiclass classification task...')
