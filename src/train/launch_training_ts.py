...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
      ...
      if task_type=='classification':
        for (inp, target) in train_dataset:
          ...
          loss=train_step_rnn_regression(inp,
                                         target,
                                         model=model,
                                         optimizer=optimizer)
      ...
      logger.info('Epoch {} - Loss {:.4f} - train acc {} - val acc {}'.format(epoch + 1, loss, train_acc, val_acc))
  ...
  if train_classic_transformer:
    ...
    ckpt = tf.train.Checkpoint(transformer=transformer,
                               optimizer=optimizer)
    baseline_ckpt_path=os.path.join(checkpoint_path,
                                    "transformer_baseline")
    ckpt_manager = tf.train.CheckpointManager(ckpt, baseline_ckpt_path, max_to_keep=EPOCHS)

    # if a checkpoint exists, restore the latest checkpoint.
    restoring_checkpoint(ckpt_manager=ckpt_manager, args=args)
